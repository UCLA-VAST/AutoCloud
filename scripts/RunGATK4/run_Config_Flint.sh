...
then
    ...
    echo "./run_Config.sh bamSize bamInput nodeNum storageConfig coreNum MEMLIMIT SPARK_MEM clearHdfs restartService comment"
    exit
fi
...
#		MEMLIMIT=$((6979321856*coreNum))
#	;;
#
#	#standard memory	
#	"1") echo "standard MEM config"
#	MEMLIMIT=$((4026531840*coreNum))
#	;;	
#
#esac
#
...
sed -- "s/SPARK_MEM/${SPARK_MEM}G/g" $SPARK_CONF_DIR/spark-env-conf${config}.sh_MEM > $SPARK_CONF_DIR/spark-env.sh
sed -i "s/SPARK_CORE_NUM/$core_num/g" $SPARK_CONF_DIR/spark-env.sh 
...
upload_bam() {
	if [ "$nodeNum" -ne "1" ]
	then
		ssh c3 "eval $UPDATE_ENV; hdfs dfs -Ddfs.replication=2 -put /hdfs0/FlintData-bam/${bamInput}";
	else
		ssh c3 "eval $UPDATE_ENV; hdfs dfs -put /hdfs0/FlintData-bam/${bamInput}";
...
change_container_memlimit() {

    for node in $CLEAN_LIST
    do
	LC_MEM=${MEMLIMIT} ssh root@${node} -p 2017 -o SendEnv=LC_MEM;
    done
	...
	do
	    ...
	    #do
			...
			#do
				...
				upload_bam;		
				clean_cache;
				$script $bamInput $core_num $SPARK_MEM;
				...
				echo "$bamInput $bamSize $node_num conf${config} $core_num $MEMLIMIT $SPARK_MEM $app_id $stage0 $stage1 $stage2 $stage8 $stage9 $stage10">> run_result.${comment}.txt;
...
main() {
    ...
    if [ -f $resultFile ]
	    ...
	    echo "bamName bamSize node_num conf core_num MEMLIMIT SPARK_MEM app_id stage0 stage1 stage2 stage8 stage9 stage10" > run_result.${comment}.txt;
    fi
